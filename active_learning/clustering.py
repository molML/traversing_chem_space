...
if __name__ == '__main__':
    ...
    smiles_index = torch.load(f'data/screen/smiles_index')
    min_supercluster_size = 128
    min_subcluster_size = 64
    n = len(smiles_screen)
    subcluster_mu = np.mean(y_screen) * min_subcluster_size
    subcluster_sigma = np.std(y_screen) * np.sqrt(min_subcluster_size)

    D = load_hdf5(f'data/screen/tanimoto_distance_vector')
    ...
    # linkage = torch.load(f'data/screen/average_linkage_clustering')

    # Cut the tree in clusters where the average intra-cluster Tanimoto distance is 0.8, 0.6
    cut_clusters = hierarchy.cut_tree(linkage, height=[0.80, 0.60])
    ...
    cluster_smiles = np.array(cluster_smiles, dtype=object)  # len = 46
    ...
    cluster_smiles_with_hits = np.array(cluster_smiles_with_hits, dtype=object)
